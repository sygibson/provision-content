...
set_tag() {
  [[ -z "$1" ]] && xiterr 1 "set_tag() failed - no tag name passed to function."
  local _tag=$(echo "$1" | sed 's/[ -:]/_/g' | tr -s '_')
  NAME_TAG="-$_tag"
  WITH_TAG="tagged '$_tag'"
}
...
case $(uname -s) in
  Darwin) ( which shasum > /dev/null 2>&1 )   && SHA="shasum -a 256" ;;
  Linux)  ( which sha256sum> /dev/null 2>&1 ) && SHA="sha256sum"     ;;
...
[[ -n "$2" ]] && set_tag $2 || { NAME_TAG=""; WITH_TAG=""; }

BLD=$(mktemp -d /tmp/esxi-bootenv-$$.XXXXX)
ISO_MNT=${BLD}.iso
...
# after fix-ups, now apply a TAG if it was provided as ARGv2
...
[[ -n "$NAME_TAG" ]] && echo "Appended custom 'tag' to Title         ::  $NAME_TAG"
...
printf "unknown" > ${BLD}/._CodeSource.meta
printf "black" > ${BLD}/._Color.meta
printf "RackN" > ${BLD}/._Copyright.meta
printf "VMware ESXi BootEnv for $TITLE $WITH_TAG" > ${BLD}/._Description.meta
printf "BootEnv for $TITLE $WITH_TAG" > ${BLD}/._DisplayName.meta
...
printf "vmware-${TITLE}${NAME_TAG}" > ${BLD}/._Name.meta
...
printf "$VER" > ${BLD}/._Version.meta

echo "Set contents Version to '$VER'"
...
echo ">>> DEBUG"
echo ""
echo "Content Pack Build directory listing:"
echo ""
cd $BLD
find . -exec ls -ld {} \;
echo ""
